...
install_github('ropensci/stplanr@6725d04b7')
```
...
You may get a warning about the graph being composed of multiple subgraphs. This is to
be expected in complex coastline areas with islands and peninsulas. The only caution is
to ensure your study area (land barrier region) is large enough such that your animal is
not restricted or, potentially, isolated in one of those subgraphs. For the vast majority
of situations, this is not going to be an issue of concern. And, likely, this warning
would be suppressed in future releases.
...
on the `nabor::knn()` function for this.
...
`dplyr::rowwise()` and `dplyr::mutate()` to draw and, then, re-route 5 imputed paths.
...
imputes <- tibble::tibble(.rows = 5) %>% 
  ...
  mutate(postis = list(crwPostIS(crw_sim, fullPost = FALSE)),
         ...
         rrt_pts = list(prt_reroute(pts, land_region, vis_graph)),
...
ggplot() +
  ggspatial::annotation_spatial(land_region, fill = "cornsilk3", size = 0) +
  ggspatial::layer_spatial(hex_grid, color = "deepskyblue", size = 0.25, fill = NA) + 
  theme_void()
```
